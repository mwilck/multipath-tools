	...
	vector_foreach_slot(blist, bl1, i) {
		...
		vector_foreach_slot_after(blist, bl2, j) {
			if (!bl1->str || !bl2->str || strcmp(bl1->str, bl2->str))
				continue;
			condlog(1, "%s: duplicate blist entry section for %s",
				__func__, bl1->str);
			free_ble(bl2);
	...
	vector_foreach_slot(blist, bl1, i) {
		...
		vector_foreach_slot_after(blist, bl2, j) {
			if ((!bl1->vendor && bl2->vendor) ||
			    ...
			    (bl1->vendor && bl2->vendor && strcmp(bl1->vendor, bl2->vendor)))
				continue;
			if ((!bl1->product && bl2->product) ||
			    (bl1->product && !bl2->product) ||
			    (bl1->product && bl2->product && strcmp(bl1->product, bl2->product)))
				continue;
			condlog(1, "%s: duplicate blist entry section for %s:%s",
